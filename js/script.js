...
window.onload = function(){
	...
	function frame2() {
		...
		function loadImages2() {
			...
			stamp.x = -300;
			stamp.y = 60;
			createjs.Tween.get( stamp, { loop: false } ).wait(1000).to( {y:-300, x:60}, 0, createjs.Ease.get(-5) ).to( { y: 50, x: 60 }, 400, createjs.Ease.get(-4) )
			.to( { y: 54, x: 60 }, 100, createjs.Ease.get(-3) ).to( { y: 50, x: 60 }, 100, createjs.Ease.get(-2) ).to( { y: 53, x: 60 }, 100, createjs.Ease.get(-1) )
			.wait(1999).to( {alpha:0}, 1 );
	...
	function frame3() {
		...
		function loadImages3() {
			...
			text5.alpha = 0;
			createjs.Tween.get( text5 ).wait(800).to( {alpha:1}, 1200 );
			...
			text6.alpha = 0;
			createjs.Tween.get( text6 ).wait(800).to( {alpha:1}, 1200 );
			...
			text7.alpha = 0;
			createjs.Tween.get( text7 ).wait(800).to( {alpha:1}, 1200 );
			...
			text8.alpha = 0;
			createjs.Tween.get( text8 ).wait(800).to( {alpha:1}, 1200 );
			...
			text9.alpha = 0;
			createjs.Tween.get( text9 ).wait(1300).to( {alpha:1}, 1200 );
			...
			text10.alpha = 0;
			createjs.Tween.get( text10 ).wait(1300).to( {alpha:1}, 1200 );
			stage.addChild( text10 );

			var text11 = new createjs.Text("Limited time offer", "19px SkyText-Regular", "#4A4A49");
			text11.x = 77;
			text11.y = 150;
			text11.textBaseline = "alphabetic";
			text11.alpha = 0;
			createjs.Tween.get( text11 ).wait(1800).to( {alpha:1}, 1200 );
			...
			text12.alpha = 0;
			createjs.Tween.get( text12 ).wait(1800).to( {alpha:1}, 1200 );
			stage.addChild( text12 );

			var shine = new createjs.Shape().set({x:145,y:221});
		  shine.graphics.beginFill("#e6e600").drawCircle(0,0,3);

		  var blurFilter = new createjs.BlurFilter(5, 5, 1);
		  shine.filters = [blurFilter];
		  var bounds = blurFilter.getBounds();

		  shine.cache(-50+bounds.x, -50+bounds.y, 100+bounds.width, 100+bounds.height);

			shine.alpha = 0;
			createjs.Tween.get( shine ).wait(2500).to( {alpha: 1}, 1 ).to( {y:221, x:145 }, 100, createjs.Ease.get(-3) )
			.to( {y:221, x:265 }, 500, createjs.Ease.get(-2) ).to( {alpha: 0}, 1 );
